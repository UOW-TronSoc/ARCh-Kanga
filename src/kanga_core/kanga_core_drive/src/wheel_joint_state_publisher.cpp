...
: Node("wheel_joint_state_publisher", options)
{
  ...
  for (size_t wheel_index = 0; wheel_index < wheel_ids_.size(); ++wheel_index) {
    const auto & wheel_id = wheel_ids_[wheel_index];
    joint_name_by_wheel_id_[wheel_id] = joint_names[wheel_index];
    joint_position_by_wheel_id_[wheel_id] = 0.0;
    joint_velocity_by_wheel_id_[wheel_id] = 0.0;
    status_received_by_wheel_id_[wheel_id] = false;
  }
...
void WheelJointStatePublisher::publish_wheel_joint_states()
{
...

  {
    ...
    for (const auto & wheel_id : wheel_ids_) {
      ...
      joint_state_message.name.push_back(joint_name_by_wheel_id_.at(wheel_id));
      joint_state_message.position.push_back(
        joint_position_by_wheel_id_.at(wheel_id));
      joint_state_message.velocity.push_back(
        joint_velocity_by_wheel_id_.at(wheel_id));
    }
  }

  // Stay quiet until at least one wheel has reported (avoids empty frames).
  if (!joint_state_message.name.empty()) {
    wheel_joint_state_publisher_->publish(joint_state_message);
  }
}
